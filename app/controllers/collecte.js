...
exports.exportData = function (req, res) {
    ...
    Collecte.find(query, '-collecte.data.gjson')
        .populate({ path: 'agent', select: 'userId' })
        .lean()
        .exec(function (err, collectes) {
            if (err) {
                return res.status(500).json(err)
            ...
            if (identification.length > 0) {
                ...
                fields.getkeys(forms[1].form).then((pel) => {
                 let fields = mainfunc(pel.components)
                //  console.log(fields)
                    for(let i = 0;i < fields.length;i++) {
                        // console.log(fields[i].type)
                        if(fields[i].type == 'radio' || fields[i].type == 'selectbox'){
                            let values = fields[i].values
                            forms[1].data.forEach(element => {
                                // console.log(values)
                                let finalvalue = values.find(e => e.value == element[fields[i].key])
                                if(finalvalue !== undefined){
                                    element[fields[i].key] = finalvalue.label
                                }
                            })
                        }
                    }
            ...
            switch (component['type']) {
                case 'columns':
                    for (let i = 0; i < component['columns'].length; i++) {
                        filtre(component['columns'][i]['components']);
                    }
                ...
                case 'table':
                    for (let i = 0; i < component['rows'].length; i++) {
                        for (let r = 0; r < component['rows'][i].length; r++) {
                            filtre(component['rows'][i][r]['components']);
                        }
                    }
